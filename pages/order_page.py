...
class OrderPage(BasePage):
    ...
    def check_order_id(self, order_id, locator):
        elements = self.check_elements_located(locator)
    ...
    def wait_loading_cross_button(self):
        self.find_element_with_wait(OrderPageLocators.CLOSE_MODAL_ORDER)
    ...
    def get_user_order_number(self, order_id):
        ...
        self.check_text_in_element(OrderPageLocators.NUMBER_ORDER_PROGRESS, order_id)
        return order_number
    ...
    @allure.step("Проверка окна заказа")
    def check_order_structure(self):
        order = self.get_text_from_element(OrderPageLocators.ORDER_STRUCTURE)
        return order == Constants.TEXT_ORDER_POPUP